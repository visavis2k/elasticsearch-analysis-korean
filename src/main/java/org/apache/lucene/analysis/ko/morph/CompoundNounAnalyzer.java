...
public class CompoundNounAnalyzer {
  ...
  private boolean divisibleOne = false;
  ...
  private TreeMap<Integer, String> findWords(int start, String input) throws MorphException {
	  ...
	  for(int i=(start+2);i<=input.length();i++) {
		  ...
		  if(iter==null || !iter.hasNext()) break;
		  
		  WordEntry entry = DictionaryUtil.getAllNoun(text);
		  if(entry!=null) {
			  List<CompoundEntry> ceList = entry.getCompounds();
			  if(ceList.size()>0) {
				  for(CompoundEntry ce : ceList) {
					  wordMap.put(ce.getWord().length(), ce.getWord());
				  }
			  } else {
